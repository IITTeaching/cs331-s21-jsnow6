...
class ArrayList:
    ...
    def insert(self, idx, value):
        ...
        for i in range(idx):
            newa[i] = self[i]
        newa[idx] = value
        for j in range(idx+1, self.len+1):
            newa[j] = self[j-1]
        self.data = newa
        self.len += 1
        ### END SOLUTION
    ...
    def __eq__(self, other):
        ...
        ### BEGIN SOLUTION
        if type(other) is not ArrayList or len(other) != self.len:
            return False
        else:
            for i in range(self.len):
                if self[i] != other[i]:
    ...
    def __add__(self, other):
        ...
        of other."""
        ### BEGIN SOLUTION
        newa = ConstrainedList(self.len + len(other))
        for i in range(self.len):
            newa[i] = self[i]
        count = 0
        for j in range(self.len, len(newa)):
            newa[j] = other[count]
            count += 1
        self.data = newa
        self.len += len(other)
